...
- (instancetype)initWithCourse:(NSInteger)course {
    ...
    if (self != nil) {
        ...
        _subjectArray = [SubjectModel subjectWithCourse:course];
        self.count = [_subjectArray count];
         [self loadUserDefault:course andChapter:-1];
        return self;
    }
    return nil;
}
...
- (instancetype)initWithCourse:(NSInteger)course Chapter:(NSInteger)chapter {
    ...
    if (self != nil) {
        ...
        _subjectArray = [SubjectModel subjectWithCourse:course ByChapter:chapter];
        self.count = [_subjectArray count];
        [self loadUserDefault:course andChapter:chapter];
        return self;
    }
    return nil;
}
...
//    UIBarButtonItem *btn0 = [[UIBarButtonItem alloc] initWithTitle:@"NO1" style:UIBarButtonItemStylePlain target:nil action:nil];
//    UIBarButtonItem *btn1 = [[UIBarButtonItem alloc] initWithTitle:@"NO2" style:UIBarButtonItemStylePlain target:nil action:nil];
//    UIBarButtonItem *btn2 = [[UIBarButtonItem alloc] initWithTitle:@"NO3" style:UIBarButtonItemStylePlain target:nil action:nil];
//    //自动计算宽度占位按钮
...
//    NSArray *array = @[btn0, space0, btn1, space0, btn2];
//    self.toolbarItems = array;
    
    //self.view.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height+self.tabBarController.tabBar.frame.size.height*2);
...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *itemright = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"readmode1"] style:UIBarButtonItemStylePlain target:self action:@selector(readMode:)];
...
- (void)readMode:(id)sender {
    if (_readType == 0) {
        _readType = 1;
        HFUTNotificationCenter *notification = [HFUTNotificationCenter notificationWithTitle:@"浏览模式" Type:HFUTNotificationInfo];
        [notification show];
        self.navigationItem.rightBarButtonItem.image = [UIImage imageNamed:@"readmode"];
        //显示答案
        [self showAnswer:0];
        [self showAnswer:1];
        [self showAnswer:-1];
        ...
        _readType = 0;
        HFUTNotificationCenter *notification = [HFUTNotificationCenter notificationWithTitle:@"练习模式" Type:HFUTNotificationInfo];
        [notification show];
        self.navigationItem.rightBarButtonItem.image = [UIImage imageNamed:@"readmode1"];
        //关闭答案
        [self dismissAnswer:0];
        [self dismissAnswer:1];
        [self dismissAnswer:-1];
...
- (void)showAnswer:(NSInteger)nums { // 0表示改变当前view，1表示改变下一个view，-1表示改变前一个view
    ...
    if (model.subjectType == 1) {
        NSString *answer = model.subjectAnswer;
        if ([answer isEqualToString:@"对"]) {
...
- (void)dismissAnswer:(NSInteger)nums {
    SubjectView *view = [_subjecViewArray objectAtIndex:(_currentSubject+2+nums)%3];
    [view openOpUse];
    SubjectModel *model = [_subjectArray objectAtIndex:(_currentSubject-1+nums+_count)%_count];
    if (model.subjectType == 1) {
        view.opA.optionMark.image = [UIImage imageNamed:@"a"];
        view.opB.optionMark.image = [UIImage imageNamed:@"b"];
    } else if (model.subjectType == 2) {
        view.opA.optionMark.image = [UIImage imageNamed:@"a"];
        view.opB.optionMark.image = [UIImage imageNamed:@"b"];
        view.opC.optionMark.image = [UIImage imageNamed:@"c"];
        view.opD.optionMark.image = [UIImage imageNamed:@"d"];
    } else {
        view.opA.optionMark.image = [UIImage imageNamed:@"a"];
        view.opB.optionMark.image = [UIImage imageNamed:@"b"];
        view.opC.optionMark.image = [UIImage imageNamed:@"c"];

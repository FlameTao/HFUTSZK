...
@interface WrongSubjectViewController () <UIViewControllerTransitioningDelegate>
...
- (void)deleteSubject:(id)sender {
    if (_count == 0) {
        ...
        HFUTAlertController * alert = [HFUTAlertController alertWithTitle:@"提示" message:@"删除错题？" style:AlertStyleDefaultWithDoubleButton];
        alert.transitioningDelegate = self;
        [alert setDefaultCompletion:^{
            SubjectModel *model = [_subjectArray objectAtIndex:(_currentSubject-1)];
            [SubjectModel deleteWrongSubject:model.subjectID andCourse:_chapter];
            _subjectArray = [SubjectModel subjectWrongWithCourse:_chapter];
            ...
            HFUTNotificationCenter *notification = [HFUTNotificationCenter notificationWithTitle:@"成功删除" Type:HFUTNotificationSuccess];
            [notification show];
        }];
